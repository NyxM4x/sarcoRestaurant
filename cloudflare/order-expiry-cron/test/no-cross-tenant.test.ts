...
describe('el destino y el nombre son los de Sarco', () => {
  ...
  it('WORKER_TICK_URL es EXACTAMENTE el tick del barrido de caducados', () => {
    // Literal completo, no un `contains`: el host correcto con la ruta de otro
    // worker seguiria siendo un despertador equivocado — y aqui ese error es
    // facil de cometer, porque este paquete nace copiado del de alertas.
    expect(raw).toContain(
      `"WORKER_TICK_URL": "https://${HOST_SARCO}/api/internal/orders/expiry/worker/tick"`,
    );
  });

  /**
   * La URL REAL configurada, extraida del valor y no del texto del archivo.
   *
   * Las comprobaciones de abajo miran esto y no el `raw` entero a proposito:
   * los comentarios de este wrangler explican de que ruta se rescato el barrido
   * —`/api/internal/cron/tick`, donde estuvo enterrado tres dias— y un
   * `not.toContain` sobre el archivo completo confundiria esa explicacion con
   * un destino. Lo que decide a quien se llama es el valor.
   */
  const destino = (raw.match(/"WORKER_TICK_URL":\s*"([^"]+)"/) ?? [])[1] ?? '';

  it('NO apunta al tick de ninguno de los otros TRES workers', () => {
    // El fallo mas probable de un paquete copiado: el nombre y los comentarios
    // cambiados, y la URL todavia apuntando al worker de origen.
    expect(destino).not.toContain('/api/internal/webhook-events/');
    expect(destino).not.toContain('/api/internal/order-notifications/');
    expect(destino).not.toContain('/api/internal/telegram-alerts/');
  });

  it('NO apunta al fallback que nadie despierta', () => {
    // `/api/internal/cron/tick` es GET y es justo donde este barrido estuvo
    // enterrado tres dias sin que nadie lo llamara. Apuntar ahi reviviria el
    // mismo fallo con una pieza nueva.
    expect(destino).not.toContain('/api/internal/cron/tick');
  });

  it('no queda ninguna otra URL absoluta en la configuracion', () => {
    // Una segunda URL seria un segundo destino, y este Worker tiene uno solo.
  ...
  it('el nombre desplegable lleva el prefijo `sarco-`', () => {
    ...
    expect(raw).toContain('"name": "sarco-order-expiry-cron"');
    ...
    expect(raw).not.toContain('sarco-telegram-alerts-cron');
  });
});
