...
const ENV: CronBindings = {
  WORKER_TICK_URL: 'https://ejemplo.test/api/internal/webhook-events/worker/tick',
...
const TICK_OK = { ok: true, claimed: 2, processed: 2, failed: 0, budget_exhausted: false };
...
describe('cron — la petición', () => {
  it('hace EXACTAMENTE un POST, con Bearer y cuerpo {}', async () => {
    ...
    await runWebhookRecoveryTick(ENV, h.deps);
  ...
  it('apunta al endpoint de webhook-events, no al de notificaciones', async () => {
    ...
    expect(h.requests[0].url).toContain('/webhook-events/worker/tick');
...
describe('cron — configuración ausente: cero POST', () => {
  it('sin URL no llama a nadie', async () => {
    const h = harness(async () => jsonResponse(200, TICK_OK));
    await runWebhookRecoveryTick({ WORKER_INTERNAL_TOKEN: 'x' }, h.deps);
  ...
  it('sin token no llama a nadie: nunca se dispara sin autenticar', async () => {
    const h = harness(async () => jsonResponse(200, TICK_OK));
    await runWebhookRecoveryTick({ WORKER_TICK_URL: ENV.WORKER_TICK_URL }, h.deps);
...
describe('cron — traducción de la respuesta', () => {
  it('200 con el contrato real: registra claimed/processed/failed', async () => {
    const h = harness(async () => jsonResponse(200, TICK_OK));
    await runWebhookRecoveryTick(ENV, h.deps);
  ...
  it('no inventa campos de order_notifications', async () => {
    const h = harness(async () => jsonResponse(200, TICK_OK));
    await runWebhookRecoveryTick(ENV, h.deps);
    ...
    // medición y no lo es.
    for (const ajeno of ['selected', 'alerts_sent', 'history_reads', 'results_count']) {
  ...
  it('un tick sin trabajo es un no-op sano, no un error', async () => {
    const h = harness(async () =>
      jsonResponse(200, { ok: true, claimed: 0, processed: 0, failed: 0, budget_exhausted: false }),
    );
    await runWebhookRecoveryTick(ENV, h.deps);
  ...
  it('presupuesto agotado se registra tal cual', async () => {
    const h = harness(async () =>
      jsonResponse(200, { ok: true, claimed: 3, processed: 3, failed: 0, budget_exhausted: true }),
    );
    await runWebhookRecoveryTick(ENV, h.deps);

    expect(h.event('cron_completed')?.fields.budget_exhausted).toBe(true);
  });
...
describe('cron — errores: se registran y NO se reintenta', () => {
  ...
  it('nunca lanza hacia el caller: un Cron que revienta no vuelve solo', async () => {
    ...
    await expect(runWebhookRecoveryTick(ENV, h.deps)).resolves.toBeUndefined();
  });
});
