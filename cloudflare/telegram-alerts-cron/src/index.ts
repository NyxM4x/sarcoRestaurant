...
/**
 * Cloudflare Module Worker — despertador del outbox de alertas de Telegram
 * (0028).
 *
 ...
 *
 * Es un despliegue INDEPENDIENTE de los otros dos crons. Ver `cron.ts` para el
 * porqué.
 */
...
export default {
  ...
  async scheduled(
    ...
    ctx.waitUntil(runAlertOutboxTick(env, defaultDeps()));
  },
  ...
  async fetch(): Promise<Response> {
    return new Response(
      JSON.stringify({ service: 'sarco-telegram-alerts-cron', status: 'ok' }),
